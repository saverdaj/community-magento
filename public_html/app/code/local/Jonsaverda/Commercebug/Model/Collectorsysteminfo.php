...
*/
class Jonsaverda_Commercebug_Model_Collectorsysteminfo extends Jonsaverda_Commercebug_Model_Observingcollector
{
    protected $_items;
    public function collectInformation($observer)
    {
        ...
        {
            Jonsaverda_Commercebug_Model_Crossareaajax::validateControllers();
        }
        
        $system_info->ajax_paths = new stdClass;
        $system_info->form_key = $this->getShim()->getSingleton('core/session')->getFormKey();
        foreach(Jonsaverda_Commercebug_Model_Crossareaajax::getAjaxActionNames() as $action)
        {
    ...
    public function createAjaxUrl($action)
    {
        ...
        #Jonsaverda_Commercebug_Model_Shim::Log('Mage2 Cleanup in ' . __CLASS__ . ' ' . __LINE__);
    ...
    */		
    public function getUrl()
    {
        ...
        #Jonsaverda_Commercebug_Model_Shim::Log('Cleanup in ' . __CLASS__ . ' ' . __LINE__);
        if($alias=='adminhtml/url' && $shim->isMage2())
        {
            $alias = 'backend/url';
        }
